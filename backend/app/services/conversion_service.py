...
def convert_files(file_ids: List[FileIDModel]) -> List[ConversionStatusModel]:
    ...
    for file_info in file_ids:
        ...
        try:
            presigned_url = generate_presigned_url(s3, settings.bucket_name, file_key)
            if not presigned_url:
                continue

            # Download the file to convert
            file_response = requests.get(presigned_url)
            temp_file_path = os.path.join(temp_dir, f"{file_id}")
        ...
        conversion_statuses.append(ConversionStatusModel(file_id=file_id, status=status))
